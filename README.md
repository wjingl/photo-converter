...
3. 在「输出设置」中调整：
   - **目标大小**（默认 50 KB）
   - **输出比例**（默认 1:1 正方形，可选 4:3 / 3:2 / 16:9 / 自定义）
   - **最长边**（默认 240px）
...
- **统一比例**：默认 1:1 正方形。原图过宽 → 居中裁左右；原图过高 → 按 **3:7 裁上下**（上方 30%、下方 70%），保留主体偏上的构图
- **统一大小**（硬约束：结果 ≤ 目标大小 ×（1+容差））：
  - **JPEG/JPG**：分辨率和质量**均由目标大小实时确定，逐图独立计算**——内容不足以达标时自动升高分辨率（上限 4096px）以承载目标大小，内容过剩时自动降分辨率并保持高质量，精确命中目标大小（±容差，默认 ±2%）
  - **PNG**：无损格式，自动调色板量化 / 降分辨率逼近目标，结果 ≤ 目标大小 ×（1+容差）
  - 极端的纯色/无纹理图像在任何分辨率下都达不到目标 → 输出其最大可达大小，并在列表中标注原因，属正常现象
- **低清增强**：源图小于目标时自动高质量上采样 + 锐化 + 自动对比度
- **分辨率自适应**：不需要手动配置像素——系统对每个文件单独计算最合适的边长（240px 起，自动升/降），以保证目标大小与画质的平衡；列表中会标注「为达目标放大至 Xpx」或「画质优先降至 Xpx」
