...
- **像素精度由目标大小实时演算**：系统为每张图独立演算恰好能承载目标大小的像素密度（DPI）——内容简单不足以承载时自动提高 DPI（上限 4096px），内容过剩时保持适当 DPI 并高质量压缩；演算结果写入文件元数据（JPEG JFIF / PNG pHYs），在任意查看器中均显示为设定的物理尺寸
- **统一大小**（硬约束：结果 ≤ 目标大小 ×（1+容差））：
  - **JPEG/JPG**：精确命中目标大小（±容差，默认 ±2%）；极简单内容达到 DPI 上限仍不达标时输出最大可达大小并标注原因
  - **PNG**：无损格式，自动调色板量化逼近目标，结果 ≤ 目标大小 ×（1+容差）
