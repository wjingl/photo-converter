...
(function () {
  ...
  function bindImport() {
    ...
    // 文件夹选择：webkitdirectory input 为主（FileList + 相对路径，兼容性最佳）；
    // showDirectoryPicker 保留备用（个别环境 webkitdirectory 弹不出目录选择器时再切换）
    ...
    dz.addEventListener('drop', async (e) => {
      ...
      try {
        ...
        if (state.items.length === before) showPageError('拖入内容中没有可用的图片' + (lastDiag ? '（诊断: ' + lastDiag.slice(0, 220) + '）' : ''));
      } catch (err) {
