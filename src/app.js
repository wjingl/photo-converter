...
(function () {
  ...
  function loadSettings() {
    try {
      ...
      if (state.settings.satMode === undefined) {
        // 旧版滑杆缓存转换（satAuto/satPct → 档位）
        if (state.settings.satAuto === false) {
          const p = state.settings.satPct || 0;
          state.settings.satMode = p <= 10 ? 'light' : p <= 20 ? 'standard' : 'strong';
        } else {
          state.settings.satMode = 'auto';
        }
      }
      if (state.settings.ditherMode === undefined) {
        if (state.settings.ditherAuto === false) {
          const p = state.settings.ditherPct || 0;
          state.settings.ditherMode = p === 0 ? 'off' : p <= 30 ? 'light' : p <= 50 ? 'standard' : 'strong';
        } else {
          state.settings.ditherMode = 'auto';
        }
      }
  ...
  function currentSettings() {
    ...
    return {
      ...
      // 高级设置档位：无 0 / 轻 0.10 / 标准 0.20 / 强 0.30
      satBoost: s.satMode === 'auto' ? (paletteColors === 8 ? 0.25 : paletteColors === 16 ? 0.22 : paletteColors === 32 ? 0.12 : 0)
  ...
  async function addDirectoryEntries(entries, baseRel = '') {
    for (const entry of entries) {
      if (entry.isFile) {
        // file() 在 file:// 下可能挂起（不回调）——加超时防卡死
        const f = await new Promise((res) => {
          let done = false;
          const finish = (v) => { if (!done) { done = true; res(v); } };
          entry.file(finish, () => finish(null));
  ...
  function showPageError(msg) {
    let bar = document.getElementById('pageError');
    if (!bar) {
      bar = document.createElement('div');
      bar.id = 'pageError';
      bar.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:200;background:#dc2626;color:#fff;padding:8px 16px;font-size:13px;text-align:center;';
      document.body.appendChild(bar);
    }
    bar.textContent = msg;
    if (errTimer) clearTimeout(errTimer);
    errTimer = setTimeout(() => { if (bar) bar.remove(); }, 8000);
  }

  // 原生目录选择器（File System Access API）：Windows 上是标准「选择文件夹」对话框；
  // file:// 属于安全上下文，Chrome/Edge 86+ 可用；不可用时回退 webkitdirectory input
  async function walkDirHandle(handle, base, files, diag) {
    let count = 0;
    ...
    while (!next.done) {
      ...
      if (entry.kind === 'file') {
        ...
        await walkDirHandle(entry, base + entry.name + '/', files, diag);
      } else {
        diag.push('  - 未知 kind');
      }
      next = await it.next();
    }
    diag.push('目录 [' + (handle.name || '?') + '] 共 ' + count + ' 个条目');
  }
  async function pickFolderViaPicker() {
    try {
      const dir = await window.showDirectoryPicker({ mode: 'read' });
      const files = [];
      const diag = [];
      // 显式确认读取权限（权限框已允许则直接 granted）
      let perm = 'granted';
      try { perm = await dir.requestPermission({ mode: 'read' }); } catch (e) { perm = '请求异常: ' + (e && e.message || e); }
      diag.push('权限=' + perm);
      await walkDirHandle(dir, '', files, diag);
      if (files.length) {
        for (const f of files) pushItem(f, f.webkitRelativePath || f.name);
        refresh();
      } else {
        // 诊断合并进最终提示（不再被覆盖）
        showPageError('未找到支持的图片（诊断: ' + diag.join(' | ').slice(0, 300) + '）');
      }
    } catch (e) {
  ...
  // 压缩包选择：showOpenFilePicker（原生文件对话框，含 ZIP 过滤器）；不可用时回退隐藏 input
  async function pickZipViaPicker() {
    try {
      ...
      if (typeof window.showOpenFilePicker !== 'function') {
        $('#zipInput').click(); // API 不可用 → 回退
      } else {
        showPageError('选择压缩包失败：' + String(e && e.message || e).slice(0, 120));
      }
    }
  }

  // 压缩包导入：按包内路径导入（保留文件夹结构）；含密码 → 抛错；单条目上限 512MB
  async function importZipFile(file) {
    ...
    refresh();
    if (importFill) importFill.style.width = '100%';
    if (!added) showPageError('压缩包中没有支持的图片（jpg/png/webp/bmp/gif/avif/svg）');
    } finally {
      if (importWrap) importWrap.hidden = true; // 无论成败都收起进度条
    }
  }

  function bindImport() {
    $('#btnPickFiles').addEventListener('click', () => $('#fileInput').click());
    // 压缩包上传：showOpenFilePicker 主路径（用户环境已验证 File System Access 可用）
    $('#btnPickZip').addEventListener('click', () => pickZipViaPicker());
    ...
    dz.addEventListener('drop', async (e) => {
      ...
      try {
        ...
        if (files.length) {
          ...
          if (handles.length) {
            ...
            if (collected.length) {
              ...
              lastDiag = diag.join(' | ');
            showPageError('拖入目录诊断: ' + lastDiag.slice(0, 220));
            }
          } else {
        ...
        if (state.items.length === before) {
          if (files.length === 0) {
            // 拖的是文件夹但句柄路径不可用 → 降级：打开目录选择器（FileList 路径最可靠）
            showPageError('浏览器不支持直接拖入文件夹，已打开文件夹选择器，请在其中选择文件夹');
            pickFolderViaPicker();
          } else {
            showPageError('拖入内容中没有可用的图片' + (lastDiag ? '（诊断: ' + lastDiag.slice(0, 220) + '）' : ''));
          }
        }
      } catch (err) {
  ...
  async function convertOneFixed(item, src, sw, sh, progress) {
    // 注意：用原始 settings（currentSettings 返回派生值，不含 colorMode/satMode/ditherMode）
    const s = state.settings;
  ...
  window.addEventListener('unhandledrejection', (ev) => {
    showPageError('后台错误：' + (ev.reason && ev.reason.message || ev.reason));
  });
